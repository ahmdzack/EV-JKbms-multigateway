/*
 * ═══════════════════════════════════════════════════════════════════════════
 * TTGO LoRa32 v2.1 — Receiver Halte
 * MQTT Publisher + Captive Portal MikroTik (TANPA SD Card)
 *
 * Penelitian: Rancang Bangun Sistem Monitoring & Predictive Maintenance
 *             Battery LiFePO4 pada Kendaraan Listrik — Komunikasi LoRa
 *
 * Versi ini = receiver_workshop dikurangi semua fitur SD Card.
 * Arsitektur WiFi/MQTT/Captive Portal dibuat IDENTIK dengan workshop supaya
 * satu basis kode mudah dirawat — perbedaan device hanya GATEWAY_ID dan
 * ada/tidaknya logging ke SD.
 *
 * [FIX] LoRa SF7/BW125 → diubah jadi SF7/BW250, disamakan dengan workshop.
 *       Versi halte sebelumnya (browser-config) pakai SF12/BW125, padahal
 *       workshop pakai SF7/BW250. Transmitter cuma broadcast SATU setting,
 *       jadi salah satu nilai itu pasti salah dan tidak bisa decode paket.
 *       Sekarang diseragamkan ke SF7/BW250 — kalau ternyata transmitter
 *       yang benar pakai SF12/BW125, nilai ini harus dibalik.
 *
 * Yang DIHAPUS dari workshop (semua terkait SD Card):
 *   — #include <FS.h>, <SD.h>
 *   — Pin SD (HSPI: SD_SCK/SD_MISO/SD_MOSI/SD_CS) & objek spiSD
 *   — CSV_FILE, REPLAY_FILE, createCSVHeader(), logToSD()
 *   — sdReady, sdPendingCount, dan field "sd"/"sd_pending" di JSON status/heartbeat
 *   — Fallback "MQTT gagal → simpan ke pending SD" — sekarang kalau publish
 *     gagal, data paket itu hilang (tidak ada media penyimpanan offline)
 *   — Dual-SPI (VSPI utk LoRa, HSPI utk SD) → balik ke SPI default karena
 *     cuma LoRa yang pakai SPI di board ini sekarang
 *
 * Yang TETAP SAMA dengan workshop:
 *   — Koneksi WiFi kampus + bypass Captive Portal MikroTik + WiFi cadangan
 *   — Re-login portal otomatis setiap 5 menit (jagaKoneksiInternet)
 *   — MQTT TLS ke HiveMQ Cloud via WiFiClientSecure + PubSubClient
 *   — Sequence counter untuk de-duplikasi
 *   — OLED 5 halaman: pack, suhu/sel, wire resistance, status, GPS
 *   — 1 node transmitter (MAX_NODES = 1)
 *
 * CATATAN: Jika node ini ada di lokasi (jaringan WiFi) yang berbeda dari
 * workshop, sesuaikan WIFI_SSID/PASSWORD, PORTAL_USER/PASS/IP, dan
 * WIFI_SSID_BK/PASS_BK di bawah. Broker MQTT (HiveMQ EU) yang sama dengan
 * workshop kemungkinan akan kena blokir IP AWS EU yang sama juga kalau
 * masih di jaringan kampus — lihat catatan migrasi ke EMQX Cloud Serverless
 * (Asia Pacific/Singapore) yang sedang dicoba di node lain.
 *
 * MQTT Topics:
...
// MQTT via WiFiClientSecure (TLS) + PubSubClient
...
void setup() {
    ...
    // ── LoRa ────────────────────────────────────────────────────────────────
    // [FIX] Tidak ada SD card di board ini → kembali ke SPI default (single
    // bus), tidak perlu dual-SPI VSPI/HSPI seperti di workshop.
    ...
    LoRa.setSpreadingFactor(7);      // [FIX] disamakan dengan transmitter & workshop
    LoRa.setSignalBandwidth(250E3);  // [FIX] disamakan dengan transmitter & workshop
